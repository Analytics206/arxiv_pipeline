...
2. **Access MongoDB** at http://localhost:27017
...
### 1. Starting Individual Containers
...
### 2. Stopping Individual Containers
...
### 3. Restarting Individual Containers
...
### 6. Managing Pipeline Service Containers

```bash
# Run the sync-mongodb pipeline container (builds if needed)
docker compose up --build sync-mongodb

# Run the sync-neo4j pipeline container (builds if needed)
docker compose up --build sync-neo4j

# Run the sync-qdrant pipeline container (builds if needed)
docker compose up --build sync-qdrant
```

### 7. Managing Monitoring Containers
...
## Monitoring Setup(optional) *Does not start with docker compose up -d* *above
...
5. **Prometheus Query Documentation**
...
- **Development Approach**:
  ...
  - Deveoper commonly uses both methods running in python env and docker compose
